...
while True:
    ...
    if mapped_predictions:
        ...
        # Draw bounding box if an elephant or pig is detected
        if label in ['elephant', 'pig']:
            arduino.write(f"{label}\n".encode())
            # Draw a small bounding box around the detected animal
            cv2.rectangle(frame, (50, 50), (200, 200), (0, 0, 255), 2)
            cv2.putText(frame, f"{label}", (60, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
